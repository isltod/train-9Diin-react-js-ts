    ...
    <main className="w-full h-full min-h-[1024px] flex gap-6 p-6">
      ...
      <section className="w-3/4 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          <Input placeholder="토픽 제목을 입력하세요." className="border-b-black h-16 pl-6 placeholder:text-lg placeholder:font-semibold"/>
        </div>
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-1">
            <Asterisk size={14} className="text-[#F96859]"/>
            ...
            <SelectContent>
              <SelectGroup>
                ...
                {TOPIC_CATEGORIES.map((item) => {
                  return (
                    <SelectItem key={item.id} value={item.label}>{item.category}</SelectItem>
                  )
                })}
              </SelectGroup>
            </SelectContent>
